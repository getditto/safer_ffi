...
impl<T> From<rust::Arc<T>>
    for Arc_<T>
{
    #[inline]
    fn from (arced: rust::Arc<T>)
      -> Arc_<T>
    {
        let raw = rust::Arc::into_raw(arced);
        Self(
            ptr::NonNull::from(unsafe { &*raw })
                .into()
        )
    }
}
...
impl<T> Arc_<T> {
      ...
      -> rust::Arc<T>
    {
        ...
        unsafe {
            rust::Arc::from_raw(this.0.as_mut_ptr())
        }
    }
}
    ...
    for Arc_<T>
{
    ...
    fn drop (self: &'_ mut Arc_<T>)
    {
        unsafe {
            drop::<rust::Arc<T>>(
                rust::Arc::from_raw(self.0.as_mut_ptr())
            );
        }
    }
}
...
pub
type Arc<T> = <T as FitForCArc>::CArcWrapped;
