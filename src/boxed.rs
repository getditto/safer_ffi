...
impl<T> From<rust::Box<T>> for Box_<T> {
    #[inline]
    fn from(boxed: rust::Box<T>) -> Box_<T> {
...
impl<T> Box_<T> {
    ...
    #[inline]
    pub fn into(self: Box_<T>) -> rust::Box<T> {
...
impl<T> DerefMut for Box_<T> {
    #[inline]
    fn deref_mut(self: &'_ mut Box_<T>) -> &'_ mut T {
        unsafe { &mut *(self.0.as_mut_ptr()) }
    }
}

unsafe impl<T> Send for Box_<T> where rust::Box<T>: Send {}

unsafe impl<T> Sync for Box_<T> where rust::Box<T>: Sync {}

impl<T: Clone> Clone for Box_<T> {
    #[inline]
...
impl<T: fmt::Debug> fmt::Debug for Box_<T> {
    fn fmt(
        self: &'_ Self,
