...
) -> Result<TokenStream2>
{
    ...
    let impl_body = {
        ...
        impl_body.extend(quote!(
            fn c_short_name_fmt (fmt: &'_ mut #ඞ::fmt::Formatter<'_>)
              -> #ඞ::fmt::Result
            {
                fmt.write_str(#EnumName_str)
            }
            ...
            {
                use #ඞ::fmt::Write as _;
                #ඞ::write!(fmt, "{}_t {}", #EnumName_str, var_name)
            }
