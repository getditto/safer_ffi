...
impl Builder<'_, WhereTo> {
    ...
    fn write_banner(
        ...
        let Some::<&str>(banner) = self.banner.or_else(|| lang.as_dyn().default_banner()) else {
            return Ok(());
        };
    ...
    fn write_prelude(
        ...
        lang.as_dyn().write_prelude(definer, self)
    }
    ...
    fn write_epilogue(
        ...
        let lang = self.get_unwrapped_language();
        lang.as_dyn().write_epilogue(definer, self)
    }
...
///
/// Mere façade over a <code>\&\'static dyn [HeaderLanguage]</code>.
#[derive(Clone, Copy)]
pub struct Language(pub &'static dyn HeaderLanguage);

impl Language {
    /// Convenience getter to be prettier than `.0`.
    pub fn as_dyn(&self) -> &'static dyn HeaderLanguage {
        self.0
    }
}
...
impl Language {
    ...
    pub const C: Self = Self(&languages::C);

    /// C#
    pub const CSharp: Self = Self(&languages::CSharp);

    /// Lua
    pub const Lua: Self = Self(&languages::Lua);

    /// A JSON file containing detailed information about the FFI declarations.
    pub const Metadata: Self = Self(&languages::Metadata);

    /// Python (experimental).
    #[cfg(feature = "python-headers")]
    pub const Python: Self = Self(&languages::Python);
}
