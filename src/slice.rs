...
impl<T> slice_raw<T> {
    ...
    ///     and immutable null-terminated slice of `len` `T`s.
    #[inline]
    pub unsafe fn as_ref<'borrow>(self: &'borrow slice_raw<T>) -> slice_ref<'borrow, T> {
        slice_ref {
            ptr: self.ptr.into(),
            len: self.len,
    ...
    ///     and immutable null-terminated slice of `len` `T`s.
    #[inline]
    pub unsafe fn as_mut<'borrow>(self: &'borrow mut slice_raw<T>) -> slice_mut<'borrow, T> {
        slice_mut {
            ptr: self.ptr.into(),
            len: self.len,
            _lt: PhantomCovariantLifetime::default(),
...
impl<'lt, T: 'lt> From<&'lt [T]> for slice_ref<'lt, T> {
    #[inline]
    fn from(slice: &'lt [T]) -> slice_ref<'lt, T> {
        slice_ref {
            len: slice.len(),
            ptr: unsafe { ptr::NonNull::new_unchecked(slice.as_ptr() as _) }.into(),
            _lt: PhantomCovariantLifetime::default(),
        }
    }
}
...
impl<'lt, T: 'lt> slice_ref<'lt, T> {
    pub fn as_slice(self: slice_ref<'lt, T>) -> &'lt [T] {
        unsafe { slice::from_raw_parts(self.ptr.as_ptr(), self.len) }
    }
}

impl<'lt, T: 'lt> Copy for slice_ref<'lt, T> {}

impl<'lt, T: 'lt> Clone for slice_ref<'lt, T> {
    #[inline]
    fn clone(self: &'_ slice_ref<'lt, T>) -> slice_ref<'lt, T> {
        *self
    }
}

impl<'lt, T: 'lt> Deref for slice_ref<'lt, T> {
    type Target = [T];

    #[inline]
    fn deref(self: &'_ slice_ref<'lt, T>) -> &'_ [T] {
        (*self).as_slice()
    }
}

unsafe impl<'lt, T: 'lt> Send for slice_ref<'lt, T> where &'lt [T]: Send {}

unsafe impl<'lt, T: 'lt> Sync for slice_ref<'lt, T> where &'lt [T]: Sync {}

impl<T: fmt::Debug> fmt::Debug for slice_ref<'_, T> {
    #[inline]
    fn fmt(
        self: &'_ Self,
...
impl<'lt, T: 'lt> From<slice_ref<'lt, T>> for slice_raw<T> {
    #[inline]
    fn from(slice_ref { ptr, len, .. }: slice_ref<'lt, T>) -> slice_raw<T> {
...
impl<'lt, T: 'lt> From<&'lt mut [T]> for slice_mut<'lt, T> {
    ...
    fn from(slice: &'lt mut [T]) -> Self {
        slice_mut {
            len: slice.len(),
            ptr: unsafe { ptr::NonNull::new_unchecked(slice.as_mut_ptr()) }.into(),
            _lt: PhantomCovariantLifetime::default(),
        }
    }
}
...
impl<'lt, T: 'lt> From<slice_mut<'lt, T>> for slice_ref<'lt, T> {
    #[inline]
    fn from(it: slice_mut<'lt, T>) -> slice_ref<'lt, T> {
        (&*it.as_slice()).into()
    }
}

impl<T> Deref for slice_mut<'_, T> {
    type Target = [T];
...
impl<T> DerefMut for slice_mut<'_, T> {
    #[inline]
...
impl<'lt, T: 'lt> slice_mut<'lt, T> {
    #[inline]
    pub fn as_ref<'reborrow>(self: &'reborrow slice_mut<'lt, T>) -> slice_ref<'reborrow, T>
    where
        'lt: 'reborrow,
    {
        let &slice_mut {
            ptr: ptr::NonNullMut(ptr, ..),
            len,
            _lt,
        } = self;
        slice_ref {
    ...
    #[inline]
    pub fn as_mut<'reborrow>(self: &'reborrow mut slice_mut<'lt, T>) -> slice_mut<'reborrow, T>
    where
        'lt: 'reborrow,
    {
        let &mut slice_mut {
    ...
    #[inline]
    pub fn as_slice(mut self: slice_mut<'lt, T>) -> &'lt mut [T] {
        unsafe { slice::from_raw_parts_mut(self.ptr.as_mut_ptr(), self.len) }
    }
}

unsafe impl<'lt, T: 'lt> Send for slice_mut<'lt, T> where &'lt mut [T]: Send {}
unsafe impl<'lt, T: 'lt> Sync for slice_mut<'lt, T> where &'lt mut [T]: Sync {}

impl<T: fmt::Debug> fmt::Debug for slice_mut<'_, T> {
    #[inline]
    fn fmt(
        self: &'_ Self,
        fmt: &'_ mut fmt::Formatter<'_>,
...
impl<'lt, T: 'lt> From<slice_mut<'lt, T>> for slice_raw<T> {
    #[inline]
    fn from(slice_mut { ptr, len, .. }: slice_mut<'lt, T>) -> slice_raw<T> {
        slice_raw { ptr: ptr.0, len }
    }
}
...
const _: () = {
    ...
    impl<T: Ord> Ord for slice_ref<'_, T> {
        #[inline]
        fn cmp(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: PartialOrd> PartialOrd for slice_ref<'_, T> {
        #[inline]
        fn partial_cmp(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: Eq> Eq for slice_ref<'_, T> {}
    impl<T: PartialEq> PartialEq for slice_ref<'_, T> {
        #[inline]
        fn eq(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: Hash> Hash for slice_ref<'_, T> {
        #[inline]
        fn hash<H: Hasher>(
            self: &'_ Self,
            hasher: &'_ mut H,
    ...
    impl<T> Default for slice_ref<'_, T> {
        #[inline]
        fn default() -> Self {
            (&[][..]).into()
        }
    ...
    impl<T: Ord> Ord for slice_mut<'_, T> {
        #[inline]
        fn cmp(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: PartialOrd> PartialOrd for slice_mut<'_, T> {
        #[inline]
        fn partial_cmp(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: Eq> Eq for slice_mut<'_, T> {}
    impl<T: PartialEq> PartialEq for slice_mut<'_, T> {
        #[inline]
        fn eq(
            self: &'_ Self,
            other: &'_ Self,
    ...
    impl<T: Hash> Hash for slice_mut<'_, T> {
        #[inline]
        fn hash<H: Hasher>(
            self: &'_ Self,
            hasher: &'_ mut H,
    ...
    impl<T> Default for slice_mut<'_, T> {
        #[inline]
        fn default() -> Self {
            (&mut [][..]).into()
        }
